...
// {
//     option.SwaggerDoc("v1", new OpenApiInfo { Title = "Ascendix API", Version = "v1" });
...
// builder.Services.AddScoped<IUserPayRepository, UserPayRepository>();
// builder.Services.AddScoped<IUserQuizAttemptRepository, UserQuizAttemptRepository>();
// builder.Services.AddScoped<IUserEarnRepository, UserEarnRepository>();
...
// FIXED: Proper authentication scheme configuration
builder.Services.AddAuthentication(options =>
{
...
.AddCookie(options =>
{
    // Cookie is only for external authentication flow
...
.AddGoogle(googleOptions =>
{
    ...
    googleOptions.ClientId = clientId;
    googleOptions.ClientSecret = clientSecret;
    // FIXED: Match the actual endpoint in your controller
    googleOptions.CallbackPath = "/api/signin-google";
...
// Add CORS if calling from a frontend
...
{
    MinimumSameSitePolicy = SameSiteMode.Lax,
    Secure = CookieSecurePolicy.Always // Use secure cookies in production
});
